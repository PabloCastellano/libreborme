...
class CSV2LBCommonParser(LBCommonParser):
    ...
    def parse_line(self, trozo):
        ...
        m = re.findall('(Reelecciones|Declaración de unipersonalidad|Nombramientos|Revocaciones|Ceses/Dimisiones|Constitución)\.\s+(.*)\.\s*' + RE_NOR_KEYWORDS, tr2_)
        ...
        m = re.findall('(Modificaciones estatutarias|Datos registrales|Cambio de denominación social|Cambio de domicilio social|Ampliacion del objeto social)\.\s+(.*)\.\s*' + RE_NOR_KEYWORDS, tr2_)
        if m != []:
            for algo in m:
                self.save_field(algo)
